#X goes to a. Y goes to b. X goes to c. Where was X before c?
...
def generate(num_sentences,names,locs,verbs,num_examples):
	...
	all_possible_combinations+=['q_BEFORE']
	...
	all_possible_combinations_transformed+=['q_BEFORE']
	...
	while len(examples) < num_examples:
		...
		#print(temp_answers, temp_locs)
		#print(qs_num, temp_names[temp_names_used[qs_num]],  temp_answers)
		pos_answers=[pa for pa in temp_answers[temp_names[temp_names_used[qs_num]]].split(';') if pa!='']
		#print(pos_answers)
		if len(pos_answers)==1:
			...
			temp_ans_transformed='LOC'+str(temp_locs.index(pos_answers[0]))+'\t'+str(qs_num+1)

			df.loc[df.index[-1],'ANSWER']=temp_answers[temp_names[temp_names_used[qs_num]]].replace('the ','') ##relation
			df_transformed.loc[df.index[-1],'ANSWER']=int(temp_locs.index(pos_answers[0])) ##relation
			temp_example+='\t'+temp_ans
			temp_example_transformed+='\t'+temp_ans_transformed
		else:
			temp_qs='Where was '+temp_names[temp_names_used[qs_num]]+' before '+pos_answers[-1]+'?'
			temp_qs_transformed='Where was '+'PER'+str(temp_names_used[qs_num])+' before '+'LOC'+str(temp_locs.index(pos_answers[-1]))+'?'
			temp_example+=temp_qs
			temp_example_transformed+=temp_qs_transformed

			df.loc[df.index[-1],'q_'+temp_names[temp_names_used[qs_num]]]=1 ##relation
			df.loc[df.index[-1],'q_BEFORE']=1
			df_transformed.loc[df.index[-1],'q_'+'PER'+str(temp_names_used[qs_num])]=1 ##relation
			df_transformed.loc[df.index[-1],'q_BEFORE']=1

			temp_ans=pos_answers[-2].replace('the ','')
			temp_ans_transformed='LOC'+str(temp_locs.index(pos_answers[-2]))

			temp_example+='\t'+temp_ans
			temp_example_transformed+='\t'+temp_ans_transformed

			df.loc[df.index[-1],'ANSWER']=temp_ans
			df_transformed.loc[df.index[-1],'ANSWER']=int(temp_locs.index(pos_answers[-2]))
